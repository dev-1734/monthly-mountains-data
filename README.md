...
[이달의 산](https://github.com/dev-1734/monthly-mountains) 앱이 읽는 **공공데이터 파이프라인**.
GitHub Actions 가 공공 API 를 호출해 정적 JSON 을 만들고 GitHub Pages 로 배포한다.
앱에는 API 키가 들어가지 않는다.

## 왜 public 인가

`dev-1734` 조직의 레포 기본값은 private 이다. 이 레포가 예외인 이유는 순서대로:

1. **GitHub Pages로 앱용 정적 JSON을 공개 배포**해야 한다.
...
네이버 관련 수집은 재배포 제약이 있어 [monthly-mountains-signals](https://github.com/dev-1734/monthly-mountains-signals)(private)
에서 돌고, **파생 산출물만** 이 레포로 push 된다.

## 현재 상태 (2026-09-14) — 조직 이전 반영 완료

인증키를 발급받아 등록했고, 미작성 스크립트도 없다. 자동 갱신이 돈다.
앱이 읽는 Pages 루트는 **`https://dev-1734.github.io/monthly-mountains-data/data/v1/`** 이다.
...
```bash
python3 pipeline/make_dev_samples.py   # 형제 디렉터리 ../monthly-mountains 에 씀
```
